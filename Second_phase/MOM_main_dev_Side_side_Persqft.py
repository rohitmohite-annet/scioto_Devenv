...
def get_data_from_config():
    ...
    InsightsJobFailureNotification = df.loc[df.SettingName == 'InsightsJobFailureNotification', 'SettingValue'].values[
        0]
...
def create_html_template(graph, current_month, useremail):
    ...
    yesfeedback = ClientEnvironmentURL + 'feedback/WWVz/' + mail_link(
        data['InsightsMasterId'].iloc[0]) + '/' + mail_link(
        data['SendToId'].iloc[0]) + '/' + mail_link(data['UserEmail'].iloc[0])
    #     print(yesfeedback)
    nofeedback = ClientEnvironmentURL + 'feedback/Tm8=/' + mail_link(
        data['InsightsMasterId'].iloc[0]) + '/' + mail_link(
        data['SendToId'].iloc[0]) + '/' + mail_link(data['UserEmail'].iloc[0])
...
def success_ran(from_mailid, to_mailid):
    ...
    <p><span style='font-size:15px;line-height:115%;font-family:"Calibri","sans-serif";'>Cron job ran successfully for Top 5 National Tenants MOM NOI Per sq.ft.</span></p> 
...
def sql_conn_fail(from_mailid, to_mailid):
    ...
    message.html_body = f'''<!DOCTYPE html>
            ...
            <p><span style='font-size:15px;line-height:115%;font-family:"Calibri","sans-serif";'>SQL server connection failed for Top 5 National Tenants MOM NOI Per sq.ft.</span></p>
            <p><br></p>
...
def cron_fail(from_mailid, to_mailid):
    ...
    <p><span style='font-size:15px;line-height:115%;font-family:"Calibri","sans-serif";'>Cron job failed for Top 5 National Tenants MOM NOI Per sq.ft. </span></p>
...
if __name__=='__main__':
    try:
        ...
        connection = sql_connection()
        data_template = pd.read_sql("select * from [dbo].[viewAllManageInsights] where InsightsMasterId = 19", connection)
        connection.close()
