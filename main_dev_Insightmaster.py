...
def fetch_data_NOI(year):
    connection = sql_connection()
    data = pd.read_sql("select * from [dbo].[viewInsightsData] where KPI = 'Net Operating Income' AND PostYear = {}".format(year), connection)
    connection.close()
    return data
...
def merge_with_sqft():
    ...
    merged_sqft = merged_sqft[['Index','Company Number','propertyname','KPI','Type','YEAR','NOI_amount','Unit Square Feet']]
    print(merged_sqft)
    return merged_sqft
